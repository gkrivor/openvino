...
TEST(iree_plugin_tests, op_concat_tensor_tensor) {
    auto test_suite = ov::iree::OperationChecker();

    auto input0 = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{3});
    auto input1 =
        std::make_shared<ov::op::v0::Constant>(ov::element::f32, ov::Shape{3}, std::vector<float>{1., 2., 3.});
    auto concat = std::make_shared<ov::op::v0::Concat>(ov::NodeVector{input0, input1}, /*axis=*/0);
    auto result0 = std::make_shared<ov::op::v0::Result>(concat);
